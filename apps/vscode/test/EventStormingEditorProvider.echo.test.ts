...
import { EventStormingEditorProvider } from '../src/EventStormingEditorProvider.js';
...
function newProvider(): EventStormingEditorProvider {
  EventStormingEditorProvider.register(context);
  return vscodeMock.lastRegisteredProvider<EventStormingEditorProvider>();
}
...
describe('EventStormingEditorProvider — echo protection', () => {
  it('posts init with the document text on `ready`', async () => {
    const doc = fakeDocument('/a.storm', 'title A\n');
